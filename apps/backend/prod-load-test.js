...
const API_URL = 'https://ew-exam-portal-backend.onrender.com';
const NUM_STUDENTS = 70;
const DURATION_SECONDS = 30;
const EXAM_CODE = 'PROD3279';

const delay = (ms) => new Promise(resolve => setTimeout(resolve, ms));

async function runStudent(index, examCode, isPerfect) {
    const studentName = isPerfect ? `Perfect Student ${index}` : `Random Student ${index}`;
    const rollNumber = `PROD-LT3-${index}`;
    let attemptId = null;
    let questions = [];
    ...
    try {
        const joinRes = await axios.post(`${API_URL}/api/exams/join`, {
            ...
            exam_code: examCode,
            department: 'Production Verification'
        });
    ...
    for (let i = 0; i < iterations; i++) {
        await delay(5000);
        // Answer all questions gradually or randomly
        const q = questions[Math.floor(Math.random() * questions.length)];
        const opts = ['A', 'B', 'C', 'D'];
        
        // If perfect, always pick 'A' since that's what we set as correct_option in our DB script
        answersMap[q.id] = isPerfect ? 'A' : opts[Math.floor(Math.random() * opts.length)];
    ...
    // Ensure perfect students answer ALL 5 questions correctly
    if (isPerfect) {
        const finalAnswers = questions.map(q => ({ questionId: q.id, selectedOption: 'A' }));
        try {
            await axios.post(`${API_URL}/api/attempts/${attemptId}/answers`, { answers: finalAnswers });
        } catch (e) {
            console.error(`[Student ${index}] Final perfect sync failed:`, e.response?.data || e.message);
        }
    }

    // 5. Final Submit via Socket with stagger
    const staggerDelay = Math.floor(Math.random() * 20000);
    console.log(`[Student ${index}] Waiting ${staggerDelay}ms before submitting...`);
        ...
        socket.once('exam_submitted', (data) => {
            if (data.success && data.result) {
                ...
                if (data.result.score === 0 && data.result.correct === 0 && data.result.wrong === 0) {
                     ...
                     console.log(`[Student ${index}] Finished. Score: ${data.result.score}/${data.result.totalMarks}`);
                     resolve(true);
                }
            } else {
                resolve(false);
...
async function startLoadTest() {
    console.log(`Starting Production Verification Load Test...`);
    console.log(`Target: ${API_URL}`);
    console.log(`Exam Code: ${EXAM_CODE}`);
    console.log(`Duration: ${DURATION_SECONDS}s`);
    console.log(`Spawning ${NUM_STUDENTS} students (10 Perfect, 60 Random)...`);
    
    const promises = [];
    for (let i = 0; i < NUM_STUDENTS; i++) {
        const isPerfect = true; // All students are perfect now
        promises.push(runStudent(i, EXAM_CODE, isPerfect));
        // Stagger connections slightly by 50ms to simulate realistic joining
        await delay(50);
    }
    ...
    if (failures > 0) {
        ...
        console.log('SUCCESS: Production System handled 70 concurrent users flawlessly.');
        process.exit(0);
    }
}
