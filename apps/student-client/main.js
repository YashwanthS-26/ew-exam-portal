...
const TASKBAR_PS_CLASS = `Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class WinBar{[DllImport("user32.dll")]public static extern IntPtr FindWindow(string a,string b);[DllImport("user32.dll")]public static extern bool ShowWindow(IntPtr h,int n);}' -ErrorAction SilentlyContinue`;
function hideTaskbar() {
    if (taskbarHidden)
        return;
    try {
        execSync(`powershell -Command "${TASKBAR_PS_CLASS};[WinBar]::ShowWindow([WinBar]::FindWindow('Shell_TrayWnd',$null),0)"`, { timeout: 3000 });
        taskbarHidden = true;
    }
    catch (_) {
...
function showTaskbar() {
    if (!taskbarHidden)
        return;
    try {
        execSync(`powershell -Command "${TASKBAR_PS_CLASS};[WinBar]::ShowWindow([WinBar]::FindWindow('Shell_TrayWnd',$null),1)"`, { timeout: 3000 });
        taskbarHidden = false;
    }
    catch (_) { }
}
function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        ...
        skipTaskbar: true, // Prevent taskbar from showing over app
        type: 'desktop', // Forces window to render below taskbar if it somehow shows
        resizable: false,
        frame: false,
        movable: false,
        maximizable: false,
...
ipcMain.on('log-violation', (_event, violationType) => {
